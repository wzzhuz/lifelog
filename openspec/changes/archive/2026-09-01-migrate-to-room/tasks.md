...
- [x] 7.1 **修复编译错误，分支编译通过**（run #81 success）
- [x] 7.2 ~~装到手机验证旧数据完整迁移~~ **已取消**：试用阶段无历史数据，迁移代码已移除
- [ ] 7.3 造 1 万条数据压测：记一笔 / 搜索 / 进设置页
      （暂未执行：需真机与批量造数，留待使用一段时间后再测）
- [x] 7.4 真机验证通过后合入主线（merge commit 8c1f4fa3，主线 #149 success）

## 8. 收尾（未完成）

- [x] 8.1 更新 `specs/data-persistence/` spec
- [x] 8.2 更新 `specs/record-capture/` 搜索部分
- [x] 8.3 修订 `project.md` 中「刻意不引入 Room」条目<br>
      （原理由是「数据量小，千级」，该前提已被证伪）
- [x] 8.4 移除 CI 中的临时日志回传步骤
- [x] 8.5 清理 `ci-logs/` 目录
- [x] 8.6 归档本 change（2026-09-01）
